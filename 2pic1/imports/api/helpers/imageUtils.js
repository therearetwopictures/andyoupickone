...
const uploadImageToGCS = fileToUpload => {
    ...
    bucket.upload(fileToUpload, (err, file) => {
        if (!err) {
...
// @returns a promise
export const downloadImage = (url, downloadedFileName) => new Promise(((resolve, reject) => {
    const options = {
        directory: 'img/',
    ...
    download(url, options, (err) => {
        if (err) {
            ...
            resolve(
                uploadImageToGCS(`${options.directory}${options.filename}`)
            );
        }
    });
}));
...
export const getGCSUrl = url => GOOGLE_STORAGE_URL + url;
