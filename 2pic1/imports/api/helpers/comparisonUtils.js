...
export const getUrl = async () => {
    ...
    while (!url) {
        ...
        let imageObj = [];
        try {
            imageObj = await imageSearch().search(
                `${seedWords} imagesize:500x500`
            );
        } catch (e) {
        ...
        // let flagWord = true;
        while (imageObj[0]) {
            const currentIndex = Math.floor(Math.random() * imageObj.length);
            let breakVar = false;
                ...
                ? await fetch(url)
                    .then(res => {
                        if (!/^image/.test(res.headers.get('content-type'))) {
                            throw new Error(
                                "not actually an image.. c'mon google!"
                            );
                        }
                        fileType = res.headers
                            .get('content-type')
                            .split('/')[1];
                        // flagWord = false;
                        breakVar = true;
                    ...
                    .catch(e => {
                        ...
                        imageObj = imageObj.reduce(
                            (acc, val, i) =>
                                (i === currentIndex ? acc : acc.concat(val)),
                            []
                        );
                    })
                : (imageObj = imageObj.reduce(
